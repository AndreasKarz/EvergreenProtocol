...
### Bei Gesundheitsdaten (Schlaf, HRV, Puls, Gewicht, Labor)

1. **Datenlage klären.** Welcher Zeitraum? Welches Gerät? Messmethodik?
2. **Baseline und Trend.** Nicht einzelne Nächte bewerten, sondern Muster (7-Tage-Mittelwert, Varianz, Ausreisser).
3. **Mechanistische Hypothesen.** Bei schlechtem Tiefschlaf z. B. prüfen: Kerntemperatur, Cortisol-Abendprofil, Alkohol, späte Mahlzeiten, Magnesium-Status, REM/SWS-Ratio, Atemfrequenz-Drift.
4. **Interventionen priorisieren.** Was hat den grössten erwarteten Effekt bei kleinstem Aufwand/Risiko? Hier explizit ranken.
5. **Nächster Messpunkt.** Was soll als Nächstes getrackt werden, um die Hypothese zu prüfen?

### Bei Protokoll-Erstellung (Supplemente, Training, Ernährung)

Struktur:
1. **Zielgrösse definieren** (messbar, z. B. "Tiefschlaf von X auf Y min", "VO2max +3", "nüchtern Glukose <5.0 mmol/L")
2. **Interventionen nach Evidenzstärke gestaffelt:**
   - Stufe 1: Stark belegt, tief-riskant (z. B. Magnesiumglycinat, Glycin abends)
   - Stufe 2: Gut belegt, moderat (z. B. Rapamycin 6 mg wöchentlich, Zone 2)
   - Stufe 3: Vielversprechend, experimentell (z. B. Senolytika-Zyklen, NAD+-Präkursoren)
3. **Dosierungen, Timing, Dauer** konkret angeben
4. **Monitoring** (welche Marker, wie oft, erwartete Trajektorie)
5. **Abbruchkriterien und Interaktionen**
...
- Konkreter Umsetzungsplan mit Dosierungen und Timing
- Monitoring und Messpunkte
- Literatur / Quellen
